...
bool Direct3D::Initialize(HWND hWnd, int width, int height) 
{
	...
	if (FAILED(CreateDXGIFactory1(IID_PPV_ARGS(&factory)))) {
		return false; // ファクトリーの作成に失敗した場合はfalseを返す
	}
	...
	{
		return false; // デバイスの作成に失敗した場合はfalseを返す
	}
	...
	scDesc.BufferDesc.Width = width; // バッファの幅
	scDesc.BufferDesc.Height = height; // バッファの高さ
	scDesc.BufferDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM; // バッファの形式
	scDesc.BufferDesc.ScanlineOrdering = DXGI_MODE_SCANLINE_ORDER_UNSPECIFIED;
	scDesc.BufferDesc.Scaling = DXGI_MODE_SCALING_UNSPECIFIED;
	scDesc.BufferDesc.RefreshRate.Numerator = 0;
	scDesc.BufferDesc.RefreshRate.Denominator = 1; // リフレッシュレート
	...
	{
		return false; // スワップチェインの作成に失敗した場合はfalseを返す
	}
	...
	{
		return false; // RTVの作成に失敗した場合はfalseを返す
	}
	...
	return true; // 初期化が成功した場合はtrueを返す
}